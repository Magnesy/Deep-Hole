        ...
        private void LineBaseGUI<SETTINGS>(SETTINGS lineSettings) where SETTINGS : LineSettings
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (lineSettings is ShapeSettings)
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var parallelToTheSurface = EditorGUILayout.ToggleLeft(
                ...
                else
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var parallelToTheSurface = EditorGUILayout.ToggleLeft("Place objects perpendicular to the "+
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (lineSettings.objectsOrientedAlongTheLine)
                {
                    ...
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var axisOrientedAlongTheLine = EditorGUILayout.Popup("Axis Oriented Along the Line:",
            ...
            if (!ToolManager.editMode)
            {
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    if (TilingManager.settings.showPreview)
                        EditorGUILayout.HelpBox("If you experience slowdown issues, disable preview.", MessageType.Info);
                }
            }
